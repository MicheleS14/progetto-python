...
#PC (salviamo su excel i dataframe)
...
r6func.show_Map_Played_Chart(df_Map_PC_Presidio, df_Map_PS4_Presidio, df_Map_XBOX_Presidio, "Presidio")

#Grafico partite Bomba
r6func.show_Map_Played_Chart(df_Map_PC_Bomba, df_Map_PS4_Bomba, df_Map_XBOX_Bomba, "Bomba")

#Grafico partite Ostaggio
r6func.show_Map_Played_Chart(df_Map_PC_Ostaggio, df_Map_PS4_Ostaggio, df_Map_XBOX_Ostaggio, "Ostaggio")
...
print(r6func.show_Operator_Pick_Win_Rate(df_Win_And_Pick_Rate_Badge))
...
with pd.ExcelWriter(current_dir + "\\File Risultato.xlsx") as writer:

    #numero partite per mappa 
    df_Match.to_excel(writer, sheet_name="Numero partite per ogni mappa", startrow=2, startcol=1)

    #partite su pc per mappa
    df_Map_PC_Presidio.to_excel(writer, sheet_name="Dataframe PC", startrow=2, startcol=1)
    df_Map_PC_Bomba.to_excel(writer, sheet_name="Dataframe PC", startrow=2, startcol=4)
    df_Map_PC_Ostaggio.to_excel(writer, sheet_name="Dataframe PC", startrow=2, startcol=7)

    #win & pickrate
    df_Win_And_Pick_Rate.to_excel(writer, sheet_name="Win & Pick Rate", startrow=2, startcol=1)

    #media kill per round
    r6func.kills_Mean_For_Round(df, significance_Level=0.05).to_excel(writer, sheet_name="Media di kill per round", startrow=2, startcol=1)

    #Probabilità di morire
    pd.DataFrame(r6func.death_Probability(df, 20, 12)).to_excel(writer, sheet_name="Probabilità di morire 12 o meno volte in 20 round", startrow=2, startcol=1)
